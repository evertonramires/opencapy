...
- The agent breaks multi-step to-dos into subtasks (on its own or when asked), placing them in a dedicated project named after the to-do so the main list stays tidy, and keeps the parent's progress bar in sync as subtasks get done — including ones you tick off directly in the Vikunja app.
